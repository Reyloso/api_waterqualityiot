...
class StaffLoginSerializer(serializers.ModelSerializer):
    class Meta:
        model = Staff
        fields = ('id','type_user', 'name', 'first_surname', 'second_surname',
             'username')
...
class DeviceLoginSerializer(serializers.ModelSerializer):
    class Meta:
        model = Device
        fields = (
            'id','type_user', 'name', 'publish_key_pubnub', 'suscribe_key_pubnub','uuid_key_pubnub','channel_pubnub','username')
...
class LoginDevice(TokenObtainPairView):
    """
    Takes a set of user credentials and returns an access and refresh JSON web
    token pair to prove the authentication of those credentials.
    ...
    def post(self, request, *args, **kwargs):
        ...
        people = Device.objects.get(username=user.username)
        if not people.status:
            return Response({'code': 2,
                             'message': 'Lo sentimos, no te encuentras activo en el sistema',
                             'data': None},
        ...
        user.last_login = timezone.now()
        user.save()
        response = super(LoginDevice, self).post(request, *args, **kwargs)
        res = response.data
        token = res.get('access')
        userinfo = DeviceLoginSerializer(people)
        return Response({'code': 1,
                         'message': 'Inicio de sesión exitoso',
                         'data': token,
                         'infodevice': userinfo.data},
                        status=200)